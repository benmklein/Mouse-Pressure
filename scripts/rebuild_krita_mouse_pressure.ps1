...
$pluginSource = Join-Path $repoRoot 'integrations\krita\mouse_pressure_brush'
$kritaSource = Join-Path $BuildRoot 'krita'
$kritaBuild = Join-Path $BuildRoot 'b_krita'
$envFile = Join-Path $BuildRoot 'env.bat'
$pluginTarget = Join-Path $kritaSource 'plugins\tools\tool_mouse_pressure'
$toolsCMake = Join-Path $kritaSource 'plugins\tools\CMakeLists.txt'
$builtPlugin = Join-Path $kritaBuild 'bin\kritatoolsmousepressure.dll'
...
$subdirectoryEntry = 'add_subdirectory( tool_mouse_pressure )'
...
$build = "call `"$envFile`" && ninja -C `"$kritaBuild`" -j8 kritatoolsmousepressure"
& cmd.exe /d /c $build
if ($LASTEXITCODE -ne 0) {
...
$distPlugin = Join-Path $distDirectory 'kritatoolsmousepressure.dll'
...
Copy-Item -LiteralPath (Join-Path $pluginSource 'mouse_pressure_brush.action') `
    -Destination (Join-Path $distDirectory 'mouse_pressure_brush.action') -Force
