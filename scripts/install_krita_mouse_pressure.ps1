...
function Send-EnvironmentChanged {
    if (-not ('MousePressureEnvironmentChangeNotifier' -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

public static class MousePressureEnvironmentChangeNotifier
{
...
'@
    ...
    $result = [UIntPtr]::Zero
    [void][MousePressureEnvironmentChangeNotifier]::SendMessageTimeout(
        [IntPtr]0xffff,
        0x001A,
        [UIntPtr]::Zero,
        'Environment',
...
if (-not $PluginPath) {
    $PluginPath = Join-Path $repoRoot 'dist\krita\5.3.3\kritatoolsmousepressure.dll'
}

$builtInPlugins = Join-Path $KritaRoot 'lib\kritaplugins'
$userPluginRoot = Join-Path $env:LOCALAPPDATA 'MousePressureKritaPlugins'
$builtInJunction = Join-Path $userPluginRoot 'builtin'
$installedPlugin = Join-Path $userPluginRoot 'kritatoolsmousepressure.dll'
$userPicsRoot = Join-Path $env:APPDATA 'krita\pics'
$userActionsRoot = Join-Path $env:APPDATA 'krita\actions'
$actionName = 'mouse_pressure_brush.action'
...
if ($Uninstall) {
    ...
    Write-Host 'Mouse Pressure Brush was uninstalled. Restart Krita.'
    return
}
...
try {
    ...
    throw "Krita is using the installed Mouse Pressure plugin. Save your work, fully exit Krita, then run this installer again."
}
