...
class MousePressureReplayHelper : public KisToolFreehandHelper
{
public:
...
KisToolMousePressure::KisToolMousePressure(KoCanvasBase *canvas)
    : KisToolFreehand(canvas,
                      KisCursor::load("tool_freehand_cursor.xpm", 2, 2),
                      kundo2_i18n("Mouse Pressure Brush Stroke"),
                      false)
{
    setObjectName("tool_mouse_pressure_brush");
    setIsOpacityPresetMode(true);

    m_replayHelper = new MousePressureReplayHelper(
...
KisToolMousePressure::~KisToolMousePressure() = default;

void KisToolMousePressure::activate(const QSet<KoShape *> &shapes)
{
...
void KisToolMousePressure::rememberEvent(KoPointerEvent *event)
{
    m_events.push_back(event->deepCopyEvent());
    m_rawPoints.push_back(convertDocumentToWidget(event->point));
}

QPointF KisToolMousePressure::documentPositionFromWidget(
...
QPointF KisToolMousePressure::imagePositionFromWidget(
    const QPointF &point) const
{
    const auto *kritaCanvas = dynamic_cast<KisCanvas2 *>(canvas());
    if (!kritaCanvas) {
...
QPointF KisToolMousePressure::pathAssistedPosition(KoPointerEvent *event,
                                                    bool begin)
{
...
qreal KisToolMousePressure::zoomAssistMultiplier() const
{
    const auto *kritaCanvas = dynamic_cast<KisCanvas2 *>(canvas());
    if (!kritaCanvas) {
        return 1.0;
...
qreal KisToolMousePressure::assistedPressure(qreal pressure,
                                              ulong timeMs,
                                              bool begin)
{
...
void KisToolMousePressure::beginPrimaryAction(KoPointerEvent *event)
{
    m_events.clear();
    m_rawPoints.clear();
...
void KisToolMousePressure::continuePrimaryAction(KoPointerEvent *event)
{
    if (m_inkMode == InkMode::PerfectInk) {
        rememberEvent(event);
    }
...
void KisToolMousePressure::replayRefinedStroke()
{
    if (m_events.empty() || m_rawPoints.isEmpty()) {
        return;
    }
    const QVector<QPointF> refined = MousePressureInkFilter::refine(
...
void KisToolMousePressure::renderPerfectFreehandStroke()
{
    if (m_events.empty() || m_rawPoints.isEmpty()) {
        return;
    }
    ...
    KisFigurePaintingToolHelper helper(
        kundo2_i18n("Mouse Pressure Filled Stroke"),
        image(),
        currentNode(),
        canvas()->resourceManager(),
...
void KisToolMousePressure::endPrimaryAction(KoPointerEvent *event)
{
    if (mode() != KisTool::PAINT_MODE) {
        m_events.clear();
        m_rawPoints.clear();
...
QWidget *KisToolMousePressure::createOptionWidget()
{
    ...
    modeCombo->addItem(i18n("Native brush + Stroke smoothing"));
    modeCombo->addItem(i18n("Experimental filled outline"));
    ...
    modeCombo->setToolTip(i18n(
        ...
        "curves. Filled outline replaces the stroke with a solid shape."));
    connect(modeCombo,
...
void KisToolMousePressure::setPressureSmoothing(qreal value)
{
...
void KisToolMousePressure::setMaximumTailLength(qreal value)
{
...
void KisToolMousePressure::setPerfectFreehandStreamline(qreal value)
{
...
void KisToolMousePressure::setPerfectFreehandSmoothing(qreal value)
{
...
void KisToolMousePressure::setPerfectFreehandThinning(qreal value)
{
