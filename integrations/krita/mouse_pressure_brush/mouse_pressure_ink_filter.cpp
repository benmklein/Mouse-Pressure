...
void MousePressureInkFilter::setMinCutoff(qreal value)
{
    m_minCutoffHz = qMax<qreal>(0.001, value);
}

void MousePressureInkFilter::setSpeedCoefficient(qreal value)
{
    m_speedCoefficient = qMax<qreal>(0.0, value);
}

void MousePressureInkFilter::reset()
{
    m_initialized = false;
    m_lastTimeMs = 0;
...
qreal MousePressureInkFilter::alpha(qreal cutoffHz, qreal dtSeconds)
{
...
QPointF MousePressureInkFilter::update(const QPointF &point, ulong timeMs)
{
    if (!m_initialized) {
        m_initialized = true;
        m_lastRaw = point;
...
QVector<QPointF> MousePressureInkFilter::refine(const QVector<QPointF> &points,
                                              qreal amount,
                                              int passes)
{
    if (points.size() < 3) {
